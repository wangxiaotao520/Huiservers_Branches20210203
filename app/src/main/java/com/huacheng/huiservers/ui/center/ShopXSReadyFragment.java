...
public class ShopXSReadyFragment extends BaseFragmentOld implements OnTabSelectListener {
    private RelativeLayout rel_no_data;
    ShopXSListNoAdapter mNoAdapter;
    RecyclerView mRecyclerView;
    SwipeRefreshLayout swipeRefreshLayout;
    private int page = 1;//当前页
    int totalPage = 0;//总页数
    ShopProtocol mProtocol = new ShopProtocol();
    List<ModelShopIndex> mBeanList = new ArrayList<>();
  ...
  //      SetTransStatus.GetStatus(getActivity());//系统栏默认为黑色
        ...
        mNoAdapter = new ShopXSListNoAdapter(getActivity(), mBeanALList);
        ...
        mRecyclerView.addOnScrollListener(new EndlessRecyclerOnScrollListener() {
            ...
            public void onLoadMore() {
                if (page<=totalPage&&!is_Requesting){
                    mNoAdapter.setLoadState(mNoAdapter.LOADING);
                    getdata(page + "",selectedPosition);
                }
            }
        });
...
//             url = info.pro_discount_list + "c_id/" + prefrenceUtil.getXiaoQuId() + "/is_star/" + "2" + "/p/" + total_Page+"/class_id/"+class_id;
//        }else {
//
//        }
        url = info.pro_discount_list  + "/is_star/" + "2" + "/p/" + total_Page+"/class_id/"+class_id+"/province_cn/"+prefrenceUtil.getProvince_cn()+"/city_cn/"+prefrenceUtil.getCity_cn()+"/region_cn/"+prefrenceUtil.getRegion_cn();
            ...
            protected void setData(String json) {
                ...
                try {
                    ...
                    if (JsonUtil.getInstance().isSuccess(jsonObject)){
                        ModelSeckill modelSeckillBean = (ModelSeckill) JsonUtil.getInstance().parseJsonFromResponse(jsonObject, ModelSeckill.class);
                        updateTag(position);
                        if (modelSeckillBean!=null&&modelSeckillBean.getClass_name()!=null){
                           ...
                           List<ModelShopIndex> list_data = modelSeckillBean.getList();
                           if (list_data!=null){
                           ...
                           if (mBeanALList.size() > 0) {
                               ...
                               if (page>totalPage){
                                   mNoAdapter.setLoadState(mNoAdapter.LOADING_END);
                               }else {
                                   mNoAdapter.setLoadState(mNoAdapter.LOADING_COMPLETE);
                               }
                               mNoAdapter.notifyDataSetChanged();
                       }
                        } else {
                            if (page==1){
    ...
    protected int getContentViewLayoutID() {
        return R.layout.shopxs_time_list_ready;
    }
