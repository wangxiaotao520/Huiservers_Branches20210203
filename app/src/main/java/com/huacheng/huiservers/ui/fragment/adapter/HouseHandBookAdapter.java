...
public class HouseHandBookAdapter extends CommonAdapter<ModelIndex> {
    ...
    protected void convert(ViewHolder viewHolder, ModelIndex item, int position) {
        GlideUtils.getInstance().glideLoad(mContext, ApiHttpClient.IMG_URL + item.getArticle_image(), viewHolder.<ImageView>getView(R.id.iv_tag), R.color.default_img_color);
