...
//    }
     ...
     *
     */
    public void getRentSellHouseList(HashMap<String, String> params){
        MyOkHttp.get().get(ApiHttpClient.GET_HOUST_LIST, params, new JsonResponseHandler() {
            ...
            public void onSuccess(int statusCode, JSONObject response) {
                if (JsonUtil.getInstance().isSuccess(response)){
                    ...
                    List<ModelMyHouseList> list=null;
                    if (myHouse!=null){
                        list = myHouse.getList();
                    }
                    listener.onGetHouseList(1,"请求成功",myHouse.getTotal_Pages(),list);

                }else {
                    String msg = JsonUtil.getInstance().getMsgFromResponse(response, "数据异常");
                    listener.onGetHouseList(0,msg,1,null);
                }
            }
            ...
            public void onFailure(int statusCode, String error_msg) {
                if (listener!=null){
                    listener.onGetHouseList(-1,"网络异常，请检查网络错误",1,null);
                }
            }
        });
    }
