    ...
    public View getView(int position, View convertView, ViewGroup parent) {
        ...
        if (type == TYPE_NORMAL) {
            ...
            if (!NullUtil.isStringEmpty(modelPhoto.getLocal_path())){
                GlideUtils.getInstance().glideLoad(mContext,modelPhoto.getLocal_path(),viewHolder.iv_img,R.color.default_img_color);
            }else {
                //显示不同比例的图片 这里显示800*1280
                GlideUtils.getInstance().glideLoad(mContext, ApiHttpClient.IMG_URL + modelPhoto.getPath(), viewHolder.iv_img, R.color.default_img_color);
            }
