package com.huacheng.huiservers.utils.custom_calendar;
...
//                    calendar.isCurrentDay() ? mCurDayLunarTextPaint : mSelectedLunarTextPaint);
            //只要选中无论是不是当天都改成选中色
           canvas.drawText(String.valueOf(calendar.getDay()), cx, mTextBaseLine + top,
                   calendar.isCurrentDay() ? mSelectTextPaint : mSelectTextPaint);
            canvas.drawText(calendar.getLunar(), cx, mTextBaseLine + y + mItemHeight / 10,
                    calendar.isCurrentDay() ? mSelectedLunarTextPaint : mSelectedLunarTextPaint);
        } else if (hasScheme) {
            canvas.drawText(String.valueOf(calendar.getDay()), cx, mTextBaseLine + top,
