...
/**
 * Created by Lydia on 2017/9/21.
 * 垂直的BannerView
 */
public class VerticalBannerView extends LinearLayout implements BaseBannerAdapter.OnDataChangedListener {
    private float mBannerHeight = TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP,32,getResources().getDisplayMetrics());
...
//            postDelayed(mRunnable,mGap);
    ...
    private void setupAdapter() {
        ...
        if(mAdapter.getCount() == 1){
            ...
            mAdapter.setItem(mFirstView,mAdapter.getItem(0),0);
            addView(mFirstView);
        }else{
            mFirstView = mAdapter.getView(this);
            mSecondView = mAdapter.getView(this);
            mAdapter.setItem(mFirstView,mAdapter.getItem(0),0);
            mAdapter.setItem(mSecondView,mAdapter.getItem(1),1);
            addView(mFirstView);
            addView(mSecondView);
    ...
    private void performSwitch(){
        ...
        set.addListener(new AnimatorListenerAdapter() {
            ...
            public void onAnimationEnd(Animator animation) {
                ...
                mPosition++;
                mAdapter.setItem(removedView,mAdapter.getItem(mPosition%mAdapter.getCount()),mPosition%mAdapter.getCount());
